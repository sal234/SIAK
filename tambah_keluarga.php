...
if(isset($_SESSION["Role_id"]) !== 0){
    echo "Anda hanyalah Kuli Surat, Anda tidak bisa menambahkan Keluarga";
    exit;
}
/*
 *      tambah_penduduk.php
	...
	var url = $(this).attr('action');
		...
		$.post(url,{data:inputs,kk:kepala_klg},function(data){
			if(data == 1){
				...
				.addClass("sukses-inline").delay("100").fadeOut("slow");
				$(".isian").val("");
				awal();
				...
				$("#status_proses").removeClass("proses-inline")
				.addClass("gagal-inline").delay("100").fadeOut("slow");
				$(".isian").val("");
				awal();
				// hapus sisa anggota keluarga
				$(".icon-del").click();
